...
public class CartController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "x-auth",value = "头信息",type = "string",required = true,paramType = "header"),
            @ApiImplicitParam(name = "goodsId",value = "ID",type = "long",required = true,paramType = "query")
    })
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "x-auth",value = "头信息",type = "string",required = true,paramType = "header"),
            @ApiImplicitParam(name = "ids",value = "要删除的id集合，如：1,2,3",type = "string",required = true,paramType = "query")
    })
    public ServerResponse batchDeleteCart(HttpServletRequest request,String ids){
        MemberVo memberVo = (MemberVo) request.getAttribute(SystemConstant.CURR_MEMBER);
        Long memberId = memberVo.getId();
        return cartService.batchDeleteCart(memberId,ids);
    }
