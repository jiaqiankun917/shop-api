...
import com.fh.shop.api.util.MD5;
...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(StringUtils.isEmpty(header)){
            throw new GlobalException(ResponseEnum.LOGIN_HEADER_LOSE);
        }
        //判断头信息是否完整
        String[] split = header.split("\\.");
        if (split.length != 2) {
            throw new GlobalException(ResponseEnum.LOGIN_HEADER_BROKEN);
        }
        //判断用户信息是否被篡改[验签]
        ...
        if (!newSignBase64.equals(signBase64)) {
            throw new GlobalException(ResponseEnum.LOGIN_MEMBER_TAMPER);
        }
        //判断是否超时[redis]
